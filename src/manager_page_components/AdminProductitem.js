...
`;
function AdminProductItem({ id, displayName, categoryName, price, img, categorySlug, createAt, status }) {
    ...
    return (
        <>
            <ToastContainer />
            <ContainerStyled onClick={handleClick} id={id} style={{backgroundColor: status===1?"rgba(6, 6, 6, 0.02)":"rgba(6, 6, 6, 0.09)"}}>
                <div style={{ flex: "1" }}>
                ...
                <div style={{ flex: "1", gap: "10px", display: "flex", alignItems: "center" }}>
                    ...
                    <div className="product-item-button">
                        <Switch checked={status === 1 ? true : false} />
                    </div>
                </div>
            </ContainerStyled>
        </>
