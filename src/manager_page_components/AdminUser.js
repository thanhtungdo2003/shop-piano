...
import { AlertCircle, Ban, Brush, Delete, Edit, EyeOff, FilePlus, FileX, Grid2X2Plus, Image, ListOrdered, Lock, LockOpen, PanelBottomIcon, PlusIcon, PlusSquare, Save, Search, Trash, Trash2, X, XCircle } from "lucide-react";
                    ...
                    <AdminElementContainer className="admin-product-container" duration={0.4}>
                        ...
                        <div className="business-cards-container">
                            {[...users].map((user, index) => {
                                return (<BusinessCard status={user.status} perm={user.permissions} username={user.user_name} email={user.email ?? "none email"} phone={user.phone ?? "none phone"} />)
                            })}
                        </div>
                    </AdminElementContainer>
                </div>
                <div style={{ display: "flex", width: "500px", height: "70%", top: "50%", left: "50%", gap: "1%", display: showUserForm ? "flex" : "none", position: "absolute", transform: "translate(-50%, -50%)" }}>
                    <AdminElementContainer className="admin-product-container" duration={0.5}>
                        <div className="category-admin-title">
                            ...
                            <div>
                                <button style={{ backgroundColor: userSelected.status === 1 ? "rgb(231, 100, 73)" : "rgb(61, 171, 219)" }}
                                    onClick={() => {
                                        axios.post(getUri() + "/user/set-status", {
                                            user_name: userSelected.user_name,
                                            status: userSelected.status === 1 ? 0 : 1
                                        }, { withCredentials: true }).then(res => {
                                            selectUser({ ...userSelected, status: userSelected.status === 1 ? 0 : 1 });

                                            userManager.onTrigger(Math.floor(Math.random() * 10));

                                        }).catch(err => {
                                            console.log(err);
                                        })
                                    }}>
                                    {userSelected.status === 1 ? <Lock color="rgb(252, 252, 252)" size="auto" /> : <LockOpen color="rgb(252, 252, 252)" size="auto" />}
                                </button>
                                <button style={{ backgroundColor: "red"}}
                                    onClick={() => {toggleUserForm(!showUserForm)}}><X color="rgb(252, 252, 252)" size="auto" />
                                </button>
                            </div>
